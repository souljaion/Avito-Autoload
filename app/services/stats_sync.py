...
async def sync_stats_for_account(account: Account, db: AsyncSession) -> dict:
    ...
    try:
        ...
        for avito_id, stats in stats_map.items():
            ...
            # Atomic upsert: INSERT ... ON CONFLICT (product_id, date) DO UPDATE
            await db.execute(text("""
                INSERT INTO item_stats (product_id, avito_id, views, contacts, favorites, price, captured_at)
                VALUES (:product_id, :avito_id, :views, :contacts, :favorites, :price, :captured_at)
                ON CONFLICT (product_id, (captured_at::date))
                DO UPDATE SET
